...
- A fila de cada conversa não vem pronta da API — é inferida em `content-script.js` (`classifyQueue`) a partir de `dateAnswer`/`dateRequest`: `dateAnswer` presente = Atendimentos (um humano já respondeu); senão `dateRequest` presente = Esperando (cliente pediu atendimento humano mas ainda não foi respondido); senão = Automático (ainda com o bot).
